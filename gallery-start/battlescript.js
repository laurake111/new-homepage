function renderGamefields(gameField, fieldName) {
    ...
    for (let y = 0; y < gameField.length; y++) {
        ...
        for (let x = 0; x < gameField.length; x++) {
            ...
            if (gameField[y][x] === "X") {
                image = "<img style=\"display:block;\" src=images/battleshipFront.png>"
            } else if (gameField[y][x] === "x") {
                image = "<img style=\"display:block;\" src=images/battleshipBack.png>"
            }
...
function boxClick(box, fieldName) {
    ...
    if (fieldName === "opponent") {
        if (opponentField[y][x].toUpperCase() === "X") {
            box.innerHTML = "<img src=images/fire2.gif>";
            opponentField[y][x] = "O";
            playerTurns++;
            playerScore++;
        }
        if (opponentField[y][x].toUpperCase() === "") {
            box.style.background = "#D78BFF";
            opponentField[y][x] = "O";
            playerTurns++;
            opponentTurn();
        }
...
function computerTryMove(x, y) {
    ...
    if (gameField[y][x].toUpperCase() === "X") {
        document.getElementById(x + "-" + y).innerHTML = "<img src=images/fire2.gif>";
        opponentScore++;
        opponentTurns++;
        gameField[y][x] = "O";
        return false;
    ...
    if (gameField[y][x].toUpperCase() === "") {
        document.getElementById(x + "-" + y).style.background = "#D78BFF";
        opponentTurns++;
        gameField[y][x] = "O";
        return true;
    }
...
function tryToPlaceShip(gameField, x, y) {
    ...
    if (x === 0 && y === 0) { // kontrollin aint alt ja paremalt 0,0
        if (gameField[y][x + 2].toUpperCase() !== "X"
            && gameField[y][x + 1].toUpperCase() !== "X"
            && gameField[y + 1][x].toUpperCase() !== "X"
    ...
    if (y === 0 && x === gameField.length - 1) { // kontrollin aint vasakult ja alt 0,2
        if (gameField[y][x - 1].toUpperCase() !== "X"
            && gameField[y][x - 2].toUpperCase() !== "X"
            && gameField[y + 1][x].toUpperCase() !== "X"
            && gameField[y + 1][x - 1].toUpperCase() !== "X") {
                gameField[y][x] = "X";
                gameField[y][x - 1] = "x";
    ...
    if (x === 0) {  //1,0
        if (y < gameField.length - 2 && y > 0 && gameField[y - 1][x].toUpperCase() !== "X"
            && gameField[y - 1][x + 1].toUpperCase() !== "X"
            && gameField[y + 1][x].toUpperCase() !== "X"
            && gameField[y + 1][x + 1].toUpperCase() !== "X"
            && gameField[y][x + 2].toUpperCase() !== "X"
    ...
    if (x === gameField.length - 1) { // 1,2
        if (y < gameField.length - 2
            && y > 0 && gameField[y - 1][x].toUpperCase() !== "X"
            && gameField[y - 1][x - 1].toUpperCase() !== "X"
            && gameField[y + 1][x].toUpperCase() !== "X"
            && gameField[y + 1][x - 1].toUpperCase() !== "X"
            && gameField[y][x - 2].toUpperCase() !== "X"
    ...
    if (y === gameField.length - 1) { // kontrollin vasakult, alt, 6levatl 2,1
        ...
        if (x < gameField.length - 2
            && x > 0 && gameField[y][x + 2].toUpperCase() !== "X"
            && gameField[y][x + 1].toUpperCase() !== "X"
            && gameField[y - 1][x].toUpperCase() !== "X"
            && gameField[y - 1][x + 1].toUpperCase() !== "X"
            && gameField[y][x - 1].toUpperCase() !== "X") {
